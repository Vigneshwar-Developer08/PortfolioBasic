    ...
    <section className="py-20 md:py-24 px-6 md:px-12 max-w-7xl mx-auto" id="skills">
      <div className="text-center mb-12">
        ...
        <h2 className="text-3xl md:text-5xl font-sans font-extrabold mb-4 text-text">
          A focused stack for <span className={`bg-gradient-to-r ${syntaxTheme === 'blue' ? 'from-indigo-600 to-indigo-800' : syntaxTheme === 'purple' ? 'from-purple-600 to-purple-800' : 'from-emerald-600 to-emerald-800'} bg-clip-text text-transparent`}>modern product delivery</span>.
        </h2>
      ...
      <div className="grid gap-6 lg:grid-cols-2 mb-10">
        {skillCategories.map((category) => (
          <div key={category.title} className="section-shell rounded-[1.5rem] p-6 md:p-7">
              ...
              {category.skills.map((skill) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full rounded-2xl border p-4 text-left shadow-sm transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                      isActive
                        ? 'border-indigo-300 bg-surface shadow-md dark:border-indigo-700'
                        : 'border-border bg-surface/70 hover:border-indigo-200 hover:bg-surface-muted dark:hover:border-indigo-800'
                    }`}
                  >
                    <div className="mb-3">
                    ...
                    <div
                      className={`overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
                        isActive ? 'mt-3 max-h-[12rem] opacity-100 translate-y-0' : 'mt-0 max-h-0 opacity-0 -translate-y-2'
                      }`}
                    >
                      <p
